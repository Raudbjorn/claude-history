...
> **Note:** Semantic search downloads the Nomic-embed-text-v1.5 model (~550 MB) to `~/.cache/claude-history/semantic/fastembed` on first run. No Python installation is required.
...
`ch_...` refs are stable local command handles derived from your history
files — they are not secrets and not a security boundary. Anyone with access
to the same history files can derive the same refs, so never treat a ref as
authorization to read a conversation.
...
Options:
  ...
  -s, --select           Enter a conversation, then press Enter again to print its session as JSON and exit
...
### Selecting a session for scripts

Use `--select` when another tool needs both the session ID and the transcript
path as JSON. The list behaves like the normal browser: `Enter` opens the
conversation preview first. Pressing `Enter` again inside the viewer confirms
the selection, prints a single JSON object to stdout, and exits. `Ctrl+O` in
the list confirms immediately without a preview.

```sh
claude-history --select > /tmp/selected.json
```

Output shape:

```json
{"sessionId":"abc123","sessionPath":"/path/to/project/abc123.jsonl"}
```

The TUI renders on stderr, so stdout stays machine-readable. `--select`
conflicts with `--show-path`, `--show-id`, `--resume`, `--plain`, `--render`,
`--delete`, and direct file input; list filters such as `--local` still work.
