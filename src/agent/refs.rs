...
mod tests {
    ...
    // ---------------------------------------------------------------------
    // Phase 3 regression suite: agent read ref resolution boundaries
    // ---------------------------------------------------------------------

    /// When the resolver finds ≥2 candidates with distinct full_refs
    /// for the same canonical `ch_` prefix, finish_resolution must
    /// report ambiguity — never silently pick one. We exercise the
    /// guard directly by feeding a synthetic match set with two keys
    /// that genuinely have different digests.
    #[test]
    fn phase3_ambiguous_ch_prefix_rejected() {
        let first = key("project-a", "11111111-1111-4111-8111-111111111111.jsonl");
        let second = key("project-b", "22222222-2222-4222-8222-222222222222.jsonl");
        let first_ref = first.conversation_ref();
        let second_ref = second.conversation_ref();
        // Sanity: digests are different (the precondition for ambiguity).
        assert_ne!(
            first_ref.full_ref(),
            second_ref.full_ref(),
            "test setup: keys must have distinct digests"
        );
        let matches = vec![
            ResolvedConversation {
                key: first,
        ...
        let err = finish_resolution("ch_collision", matches)
            .expect_err("distinct-ref match set must error as ambiguous");
    ...
    /// Focus m20 with range m5..m10 is invalid: focus must be a subset
    /// of the requested range. validate_resolved_focus_in_ranges is the
    /// single source of truth for that rule.
    #[test]
    fn phase3_focus_outside_requested_range_rejected() {
        let reads = [parse_read_ref("ch_12345678:m5..m10").unwrap()];
        let resolved = ResolvedConversation {
            key: key("project-a", "12345678-1234-4234-9234-123456789abc.jsonl"),
            reference: AgentConversationRef::from_parts(
                "project-a",
                "12345678-1234-4234-9234-123456789abc.jsonl",
            ),
        };
        ...
        let err = validate_resolved_focus_in_ranges(
            &resolved_reads,
            &parse_focus_ref("m20").unwrap(),
            None,
        )
        .expect_err("focus outside range must error");
        let msg = err.to_string();
        assert!(
            msg.contains("outside"),
            "error should mention 'outside the requested read range', got: {msg}"
        );
    }

    /// Focus m7 with range m5..m10 is valid; the validator must accept it.
    #[test]
    fn phase3_focus_within_requested_range_succeeds() {
        let reads = [parse_read_ref("ch_12345678:m5..m10").unwrap()];
        let resolved = ResolvedConversation {
            key: key("project-a", "12345678-1234-4234-9234-123456789abc.jsonl"),
            reference: AgentConversationRef::from_parts(
