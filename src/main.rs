...
fn run() -> Result<()> {
    ...
    if args.select {
        let output = selected_conversation_json(&selected_path)?;
        println!("{}", output);
        return Ok(());
    }
...
/// Generate JSON output for the selected conversation in select mode
fn selected_conversation_json(selected_path: &Path) -> Result<String> {
...
mod select_mode_tests {
    ...
    #[test]
    fn selected_conversation_json_valid_uuid() {
        let path = PathBuf::from("/path/to/abc123.jsonl");
        let result = selected_conversation_json(&path).unwrap();
        let parsed: serde_json::Value = serde_json::from_str(&result).unwrap();
        assert_eq!(parsed["sessionId"], "abc123");
        assert_eq!(parsed["sessionPath"], "/path/to/abc123.jsonl");
    }

    #[test]
    fn selected_conversation_json_path_with_spaces() {
        let path = PathBuf::from("/my project/session.jsonl");
        let result = selected_conversation_json(&path).unwrap();
        let parsed: serde_json::Value = serde_json::from_str(&result).unwrap();
        assert_eq!(parsed["sessionId"], "session");
        assert_eq!(parsed["sessionPath"], "/my project/session.jsonl");
    }

    #[test]
    fn selected_conversation_json_non_unicode_filename() {
        use std::ffi::OsStr;
        use std::os::unix::ffi::OsStrExt;
        let invalid_bytes: &[u8] = b"\xff\xfe";
        let invalid_os_str = OsStr::from_bytes(invalid_bytes);
        let mut path = PathBuf::from("/path/to/");
        path.push(invalid_os_str);
        let result = selected_conversation_json(&path);
