...
#[test]
fn rendered_tool_output_defaults_to_no_subagent_action() {
    let output_id = make_tool_output_id(0, None, 0, ToolOutputKind::ToolResult, Some("toolu"));
    let line = RenderedLine::tool_output(Vec::new(), output_id, true);

    assert!(line.subagent_tool_use_id.is_none());
}

#[test]
fn linked_task_header_is_clickable() {
    let task_id = "toolu_task";
    let entries = vec![RenderableEntry {
        entry_index: 0,
        entry: serde_json::from_str(
            r#"{"type":"assistant","message":{"role":"assistant","content":[{"type":"tool_use","id":"toolu_task","name":"Task","input":{"description":"linked task"}}]}}"#,
        )
        .unwrap(),
    }];
    let mut options = test_render_options(ToolDisplayMode::Truncated);
    options.subagent_link_ids.insert(task_id.to_string());

    let rendered = render_parsed_conversation(&entries, &options);
    let header = rendered
        .lines
        .iter()
        .find(|line| line_text(line).contains("[→ subagent]"))
        .expect("linked task header");
    assert!(header.clickable);
    assert_eq!(header.subagent_tool_use_id.as_deref(), Some(task_id));
}

#[test]
fn linked_task_summary_uses_tool_call_id_for_navigation() {
    let task_id = "toolu_task";
    let entries = vec![RenderableEntry {
        entry_index: 0,
        entry: serde_json::from_str(
    ...
    let summary = rendered
        ...
        .iter()
        .find(|line| line_text(line).contains("[→ subagent]"))
        .expect("linked task summary");
    assert!(summary.clickable);
    assert_eq!(summary.subagent_tool_use_id.as_deref(), Some(task_id));
}

#[test]
fn mixed_content_linked_task_summary_attaches_navigation_action() {
    let task_id = "toolu_task";
    let entries = vec![RenderableEntry {
        entry_index: 0,
        entry: serde_json::from_str(
            r#"{"type":"assistant","message":{"role":"assistant","content":[{"type":"text","text":"Planning the work."},{"type":"tool_use","id":"toolu_task","name":"Task","input":{"description":"linked task"}}]}}"#,
        )
        .unwrap(),
    }];
    let mut options = test_render_options(ToolDisplayMode::Hidden);
    ...
    let summary = rendered
        ...
        .iter()
        .find(|line| line_text(line).contains("[→ subagent]"))
        .expect("linked mixed-content summary");
    assert!(summary.clickable);
    assert_eq!(summary.subagent_tool_use_id.as_deref(), Some(task_id));
}

#[test]
fn unlinked_task_summary_omits_subagent_affordance() {
    let entries = vec![RenderableEntry {
        entry_index: 0,
        entry: serde_json::from_str(
            r#"{"type":"assistant","message":{"role":"assistant","content":[{"type":"tool_use","id":"toolu_task","name":"Task","input":{"description":"unlinked task"}}]}}"#,
        )
        .unwrap(),
    }];
    ...
    assert!(text.contains("Dispatched agent: unlinked task"));
}
...
fn message_range_helper_excludes_trailing_blank() {
    ...
    let r = message_range_excluding_trailing_blank(&lines, 0, 2, 7, 7).unwrap();
...
fn message_range_helper_returns_none_for_empty_slice() {
    ...
    assert!(message_range_excluding_trailing_blank(&lines, 1, 1, 0, 0).is_none());
}

#[test]
fn message_range_helper_returns_none_when_only_trailing_blank() {
    let lines = vec![blank_line()];
    assert!(message_range_excluding_trailing_blank(&lines, 0, 1, 0, 0).is_none());
}
...
#[test]
fn subagent_links_discovery() {
    use super::{RenderableEntry, scan_subagent_dir};
    use crate::claude::LogEntry;
    use crate::claude::{AssistantMessage, ContentBlock};
    use tempfile::TempDir;

    let dir = TempDir::new().unwrap();
    let project_dir = dir.path();

    // Create session uuid dir + subagents subdir
    ...
    // agent file named agent-<prefix>.jsonl where prefix matches tool_use_id start
    let agent_id = "a1b2c3d4e5f6";
    std::fs::write(subagents_dir.join(format!("agent-{}.jsonl", agent_id)), b"").unwrap();

    let jsonl_path = project_dir.join(format!("{}.jsonl", session_uuid));

    // Build a minimal parsed entry containing a Task ToolUse with id starting with agent_id
    let full_tool_use_id = format!("{}xyz", agent_id); // longer than the file prefix
    let entry = RenderableEntry {
        entry_index: 0,
    ...
    let links = scan_subagent_dir(&jsonl_path, &entries);
    assert_eq!(links.len(), 1);
    assert!(links.contains_key(&full_tool_use_id));
}

#[test]
fn nested_subagent_discovery_reuses_current_subagents_directory() {
    use super::{RenderableEntry, scan_subagent_dir};
    use crate::claude::LogEntry;
    use crate::claude::{AssistantMessage, ContentBlock};
    use tempfile::TempDir;
    ...
    let subagents_dir = dir
        .path()
        .join("aaaabbbb-cccc-dddd-eeee-ffffffffffff")
        .join("subagents");
    std::fs::create_dir_all(&subagents_dir).unwrap();
    let current_agent_path = subagents_dir.join("agent-parent.jsonl");
    let nested_agent_path = subagents_dir.join("agent-child.jsonl");
    std::fs::write(&current_agent_path, b"").unwrap();
    std::fs::write(&nested_agent_path, b"").unwrap();
    std::fs::write(
        subagents_dir.join("agent-child.meta.json"),
        r#"{"toolUseId":"agent-child-tool","description":"nested task","spawnDepth":2}"#,
    )
    .unwrap();

    let nested_tool_use_id = "agent-child-tool".to_string();
    let entries = vec![RenderableEntry {
        entry_index: 0,
        entry: LogEntry::Assistant {
            message: AssistantMessage {
                role: "assistant".into(),
                content: vec![ContentBlock::ToolUse {
                    id: nested_tool_use_id.clone(),
                    name: "Agent".into(),
                    input: serde_json::json!({"description": "nested task"}),
                }],
                model: None,
                usage: None,
                id: None,
    ...
    let links = scan_subagent_dir(&current_agent_path, &entries);
    assert_eq!(links.get(&nested_tool_use_id), Some(&nested_agent_path));
}
...
#[test]
fn subagent_links_longest_prefix_wins() {
    use super::{RenderableEntry, scan_subagent_dir};
    use crate::claude::LogEntry;
    use crate::claude::{AssistantMessage, ContentBlock};
    use tempfile::TempDir;
    ...
    // Two files whose file_ids are both prefixes of the same tool_use_id.
    // "ab" is a 2-char prefix; "abc" is a 3-char prefix. The longer one wins.
    std::fs::write(subagents_dir.join("agent-ab.jsonl"), b"").unwrap();
    std::fs::write(subagents_dir.join("agent-abc.jsonl"), b"").unwrap();

    let jsonl_path = project_dir.join(format!("{}.jsonl", session_uuid));

    let full_tool_use_id = "abcxyz";
    let entry = RenderableEntry {
        entry_index: 0,
        entry: LogEntry::Assistant {
            message: AssistantMessage {
    ...
    assert_eq!(
        links.len(),
        1,
        "exactly one link expected, got {:?}",
        links.keys().collect::<Vec<_>>()
    );
    let expected = subagents_dir.join("agent-abc.jsonl");
    assert_eq!(
        links.get("abcxyz"),
        Some(&expected),
...
#[test]
fn subagent_links_non_task_agent_ignored() {
    use super::{RenderableEntry, scan_subagent_dir};
    use crate::claude::LogEntry;
    use crate::claude::{AssistantMessage, ContentBlock};
    use tempfile::TempDir;
    ...
    // An agent file exists, but the only ToolUse is a "Bash" tool (not Task/Agent).
    std::fs::write(subagents_dir.join("agent-abc.jsonl"), b"").unwrap();
    let jsonl_path = project_dir.join(format!("{}.jsonl", session_uuid));
    ...
    assert!(
        links.is_empty(),
        "non-Task/Agent tools must not link: {:?}",
        links.keys().collect::<Vec<_>>()
    );
}
