...
use super::summary::{
    ToolActivitySummaryInteraction, render_tool_activity_summary, summarize_tool_calls,
};
use super::timing::{RowTiming, TimingSlot};
use super::tools::{
    ToolCallRenderSpec, ToolOutputKind, ToolResultRenderSpec, format_tool_result_content,
    make_tool_output_id, make_tool_summary_output_id, render_dimmed_tool_result_body,
    render_subagent_tool_result_header, render_tool_call, render_tool_result,
...
fn step_tool_summary(
    ...
    let summary = summarize_tool_calls(blocks, ctx.options);
    if summary.is_empty() {
        return false;
    }
...
fn step_tool_calls(
    ...
    for (block_idx, block) in blocks.iter().enumerate() {
        ...
        let has_subagent = has_subagent_link(name, id, ctx.options);
