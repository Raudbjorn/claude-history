...
fn write_linked_task_conversation(
    path: &std::path::Path,
    task_id: &str,
    prepend_bash: bool,
) -> PathBuf {
    ...
    let session_id = path.file_stem().and_then(|stem| stem.to_str()).unwrap();
    let subagents_dir = path.parent().unwrap().join(session_id).join("subagents");
    std::fs::create_dir_all(&subagents_dir).unwrap();
    let agent_path = subagents_dir.join(format!("agent-{task_id}.jsonl"));
    write_named_conversation(&agent_path, "subagent body");
    agent_path
}

fn subagent_link_row(app: &App, frame: Rect) -> u16 {
    ...
    let idx = state
        ...
        .position(|line| {
            line.spans
                .iter()
                .any(|(text, _)| text.contains("[→ subagent]"))
        })
        .unwrap();
    assert!(state.rendered_lines[idx].clickable);
    layout.content.y + (idx - state.scroll_offset) as u16
}

fn truncation_row(app: &App, frame: Rect) -> u16 {
    let Some(state) = app.view_state() else {
        unreachable!()
    };
    let layout = ui::view_layout_rects(frame, app, state);
    let idx = state
        ...
        .position(|line| {
            line.spans
                .iter()
                .any(|(text, _)| text.contains("more lines"))
        })
        .unwrap();
    assert!(state.rendered_lines[idx].clickable);
    layout.content.y + (idx - state.scroll_offset) as u16
...
fn semantic_ranked_selection_opens_selected_conversation_and_returns() {
    ...
    app.enter_view_mode(80);
    assert!(matches!(app.app_mode(), AppMode::View { .. }));
    if let Some(state) = app.view_state() {
...
fn view_hover_tracks_clickable_output() {
    ...
    let mut app = app_with_tool_conversation(path);
    let frame = Rect::new(0, 0, 120, 20);
    let (row, id) = if let Some(state) = app.view_state() {
        let layout = ui::view_layout_rects(frame, &app, state);
        let idx = state
            .rendered_lines
            .iter()
...
#[test]
fn single_file_linked_task_click_opens_subagent() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir
        .path()
        .join("aaaabbbb-cccc-dddd-eeee-ffffffffffff.jsonl");
    let task_id = "task-abc123";
    let agent_path = write_linked_task_conversation(&path, task_id, false);
    let mut app = App::new_single_file(
        path,
        ToolDisplayMode::Truncated,
        false,
    ...
    assert!(app.handle_view_click(subagent_link_row(&app, frame), frame, 17));
    assert_eq!(app.view_stack_depth(), 2);
    assert_eq!(app.view_state().unwrap().conversation_path, agent_path);
}

#[test]
fn summary_group_enter_scans_later_linked_task() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir
        .path()
        .join("aaaabbbb-cccc-dddd-eeee-ffffffffffff.jsonl");
    let task_id = "task-abc123";
    let agent_path = write_linked_task_conversation(&path, task_id, true);
    let mut app = App::new(
        vec![test_conversation(path, None)],
        ToolDisplayMode::Hidden,
        false,
    ...
    app.handle_key(KeyCode::Enter, KeyModifiers::empty(), 17);
    app.handle_key(KeyCode::Enter, KeyModifiers::empty(), 17);
...
#[test]
fn expanded_mixed_summary_linked_task_click_opens_subagent() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir
        .path()
        .join("aaaabbbb-cccc-dddd-eeee-ffffffffffff.jsonl");
...
#[test]
fn collapsed_summary_linked_task_click_opens_subagent() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir
        .path()
        .join("aaaabbbb-cccc-dddd-eeee-ffffffffffff.jsonl");
...
#[test]
fn linked_task_truncation_click_expands_without_entering_subagent() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir
        .path()
        .join("aaaabbbb-cccc-dddd-eeee-ffffffffffff.jsonl");
    let task_id = "task-abc123";
    let _agent_path = write_linked_task_conversation(&path, task_id, false);
    let mut app = App::new_single_file(
        path,
        ToolDisplayMode::Truncated,
        false,
    ...
    assert!(app.handle_view_click(truncation_row(&app, frame), frame, 17));
    assert_eq!(app.view_stack_depth(), 1);
    assert!(view_text(&app).contains("line five"));
}
