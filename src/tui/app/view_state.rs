...
impl App {
    pub fn enter_view_mode(&mut self, content_width: usize) {
        use crate::tui::viewer::{
            RenderOptions, parse_conversation_file, render_parsed_conversation, scan_subagent_dir,
        };
        ...
        match parse_conversation_file(&path) {
            Ok(entries) => {
                let subagent_links = scan_subagent_dir(&path, &entries);
                let subagent_link_ids = subagent_links.keys().cloned().collect();
    ...
    pub fn push_view(&mut self, agent_path: PathBuf, tool_use_id: String) -> bool {
        use crate::tui::viewer::{
            LineStyle, RenderOptions, RenderedLine, parse_conversation_file,
            render_parsed_conversation, scan_subagent_dir,
        };

        let content_width = self
            .view_state()
            .map(|state| state.content_width)
            .unwrap_or(80);
        let tool_display = self
            .view_state()
            .map(|state| state.tool_display)
            .unwrap_or(self.tool_display);
        let show_thinking = self.show_thinking;
        let show_timing = self.show_timing;

        match parse_conversation_file(&agent_path) {
            Ok(entries) => {
                let subagent_links = scan_subagent_dir(&agent_path, &entries);
                let subagent_link_ids = subagent_links.keys().cloned().collect();
                let entries = Arc::new(entries);
                let options = RenderOptions {
                    tool_display,
                ...
                let header_line = RenderedLine::new(vec![(
                    header_text,
                    LineStyle {
                        fg: Some(crate::tui::theme::detect_theme().accent_dim),
                        dimmed: true,
                        ..Default::default()
                    },
                )]);
                let mut rendered_lines = rendered.lines;
                rendered_lines.insert(0, header_line);

                let mut message_ranges = rendered.messages;
                for range in &mut message_ranges {
                    range.start_line += 1;
                    range.end_line += 1;
                }
                let total_lines = rendered_lines.len();
                let first_msg = if message_ranges.is_empty() {
                    None
                } else {
                    Some(0)
                };
                ...
                let new_state = ViewState {
                    ...
                    hovered_tool_output: None,
                    subagent_links,
                    entered_from: Some(tool_use_id),
                };
                if let AppMode::View { stack } = &mut self.app_mode {
            ...
            Err(e) => {
                self.status_message = Some((
                    format!("Failed to open subagent file: {}", e),
    ...
    pub(super) fn re_render_view(&mut self, viewport_height: usize) {
        use crate::tui::viewer::{
            parse_conversation_file, render_parsed_conversation, scan_subagent_dir,
        };
        ...
        if let Some(state) = self.view_state_mut() {
            ...
            let entries = match state.parsed_entries.clone() {
                ...
                None => match parse_conversation_file(&state.conversation_path) {
                    Ok(entries) => {
                        if state.subagent_links.is_empty() {
                            state.subagent_links =
                                scan_subagent_dir(&state.conversation_path, &entries);
                        }
                        let entries = Arc::new(entries);
            ...
            let options = RenderOptions {
                ...
                subagent_link_ids: state.subagent_links.keys().cloned().collect(),
            };
            let rendered = render_parsed_conversation(&entries, &options);
    ...
    pub fn scroll_mouse(&mut self, delta: isize, viewport_height: usize) {
        ...
        match self.app_mode {
            ...
            AppMode::View { .. } => self.scroll_view(delta, viewport_height),
        }
    }
    ...
    pub fn handle_view_click(
        ...
        let message_idx = self
            .view_state()
            .and_then(|state| Self::message_idx_at_line(&state.message_ranges, line_idx));
        if tool_output.is_none() && message_idx.is_none() {
            return false;
        }

        let subagent_entry = self.view_state().and_then(|state| {
            let tool_use_id = state
                .rendered_lines
                .get(line_idx)?
                .subagent_tool_use_id
                .as_ref()?;
            let agent_path = state.subagent_links.get(tool_use_id)?.clone();
            Some((agent_path, tool_use_id.clone()))
        });
        if let Some((agent_path, tool_use_id)) = subagent_entry {
    ...
    pub(super) fn sync_focus_to_scroll(state: &mut ViewState, viewport_height: usize) {
        if state.message_ranges.is_empty() {
            return;
        }
    ...
    pub(super) fn copy_focused_message(&mut self, viewport_height: usize) {
        ...
        let (path, entry_index) = if let Some(state) = self.view_state() {
            if let Some(idx) = state.focused_message {
        ...
        let options = if let Some(state) = self.view_state() {
