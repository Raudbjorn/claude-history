...
#[test]
fn finish_search_loading_hydrates_semantic_snapshot_before_dispatch() {
    let (mut app, request_rx, _response_tx) =
        app_with_single_visible_conversation_and_semantic_worker();
    let path = app.conversations[0].path.clone();
    app.conversations[0].full_text.clear();
    app.conversations[0].search_text_lower.clear();
    app.conversations_snapshot = Arc::new(app.conversations.clone());
    app.rebuild_semantic_conversations_snapshot();
    app.apply_search_data(vec![(path, "hydrated body needle".to_string())]);
    app.query = "\"hydrated body needle\"".to_string();

    app.finish_search_loading();
    ...
    let corpus = commands
        ...
        .expect("hydrated semantic corpus");
    let update_index = commands
        .iter()
        .position(|command| matches!(command, SemanticWorkerCommand::UpdateCorpus { .. }))
        .expect("hydrated semantic corpus command");
    let search_index = commands
        .iter()
        .position(|command| matches!(command, SemanticWorkerCommand::Search { .. }))
        .expect("semantic search command");
    assert!(
        update_index < search_index,
        "semantic corpus must update before the rerun search"
    );
    assert_eq!(request.1, "\"hydrated body needle\"");
    assert_eq!(app.semantic_search.pending_generation, Some(request.0));
    assert_eq!(corpus[0].full_text, "hydrated body needle");
}
...
fn select_mode_enter_in_view_returns_select_action() {
    ...
    // Create a temporary directory and real JSONL file
    let temp_dir = tempfile::tempdir().unwrap();
    let jsonl_path = temp_dir.path().join("test-session.jsonl");
    fs::write(
        &jsonl_path,
    ...
    let conv = Conversation {
        ...
        preview_first: "hello".to_string(),
        preview_last: "hello".to_string(),
        full_text: "hello".to_string(),
        agent_search_text: String::new(),
        semantic_turns: vec!["hello".to_string()],
        semantic_turn_ranges: vec![crate::agent::refs::MessageRange::single(1)],
        search_text_lower: search::normalize_for_search("hello"),
    ...
    app.append_conversations(vec![conv]);
    app.finish_loading();
    app.selected = Some(0); // Ensure selection is set

    // Enter view mode (this will parse the real file)
    app.enter_view_mode(80);

    // Press Enter should return Select action with the conversation path
    let action = app.handle_key(KeyCode::Enter, KeyModifiers::NONE, 10);
    match action {
        Some(Action::Select(path)) => {
            assert_eq!(path, jsonl_path);
        }
        other => panic!("Expected Action::Select, got {:?}", other),
    }
}

#[test]
fn non_select_mode_enter_in_view_does_not_return_select() {
    use std::fs;
    ...
    // Press Enter should NOT return Select action (it handles message navigation)
    let action = app.handle_key(KeyCode::Enter, KeyModifiers::NONE, 10);
    assert!(action.is_none(), "Expected None, got {:?}", action);
}

#[test]
fn select_mode_list_enter_does_not_select() {
    use std::fs;

    // In select mode, Enter must only confirm selection from *within* the
    // viewer. From the list, Enter opens the preview (handled by the runtime
    // event loop, which is unaffected by select_mode) and must never
    // short-circuit into an immediate Action::Select.
    let temp_dir = tempfile::tempdir().unwrap();
    let jsonl_path = temp_dir.path().join("test-session.jsonl");
    fs::write(
        &jsonl_path,
    ...
    // Stay in List mode (do NOT enter view mode). Pressing Enter must not
    // return Action::Select; the select action is confined to the viewer.
    assert!(matches!(app.app_mode(), AppMode::List));
    let action = app.handle_key(KeyCode::Enter, KeyModifiers::NONE, 10);
    assert!(
        !matches!(action, Some(Action::Select(_))),
        "list-mode Enter must not select in select mode, got {:?}",
        action
    );
}

#[test]
fn push_view_missing_file_does_not_crash() {
    use std::fs;

    // Create a real session file so the app can enter view mode normally
    let temp_dir = tempfile::tempdir().unwrap();
    let jsonl_path = temp_dir.path().join("test-session.jsonl");
    fs::write(
        &jsonl_path,
    ...
    // Sanity: we're in view mode with one frame on the stack
    assert_eq!(app.view_stack_depth(), 1, "expected to be in view mode");
    assert_eq!(app.status_message(), None, "no status message yet");

    // Try to push a view pointing at a nonexistent file
    let nonexistent = temp_dir.path().join("does-not-exist.jsonl");
    let result = app.push_view(nonexistent.clone(), "fake-tool-use-id".to_string());

    // Push should fail gracefully (return false) without panicking
    assert!(!result, "push_view should return false for missing file");

    // Stack must be unchanged
    assert_eq!(
        app.view_stack_depth(),
        1,
        "stack must not grow when file is missing"
    );

    // A status message must be set so the user sees what went wrong
    let status = app
        .status_message()
        .expect("status message must be set on failure");
    assert!(
        status.0.contains("Failed to open subagent") || status.0.contains("not found"),
        "status message should explain the failure, got: {:?}",
        status.0
    );
}
