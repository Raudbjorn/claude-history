...
pub struct App {
    ...
    /// Whether select mode is active (Enter in viewer emits JSON and exits)
    select_mode: bool,
}
...
impl App {
    ...
    /// Store full_text / search_text_lower from a SearchBatch — no rebuild yet.
    pub fn apply_search_data(&mut self, pairs: Vec<(PathBuf, String)>) {
        for (path, full_text) in pairs {
            if let Some(conv) = self.conversations.iter_mut().find(|c| c.path == path) {
                conv.search_text_lower = search::normalize_for_search(&full_text);
                conv.full_text = full_text;
            }
        }
    }

    /// Called after all SearchBatches arrive: rebuild the search index and re-run
    /// any active query so body search now works.
    pub fn finish_search_loading(&mut self) {
        self.searchable = search::precompute_search_text(&self.conversations);
        self.conversations_snapshot = Arc::new(self.conversations.clone());
        self.rebuild_semantic_conversations_snapshot();
        let _ = self.search_tx.send(SearchCommand::UpdateData {
    ...
    pub fn view_state_mut(&mut self) -> Option<&mut ViewState> {
        match &mut self.app_mode {
            AppMode::View { stack } => stack.last_mut(),
            AppMode::List => None,
        }
    }

    pub fn view_stack_depth(&self) -> usize {
        match &self.app_mode {
            AppMode::View { stack } => stack.len(),
            AppMode::List => 0,
        }
    }
