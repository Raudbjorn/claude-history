...
fn prepare_frame(app: &mut App, terminal: &mut Terminal<CrosstermBackend<Stderr>>) -> FrameState {
    ...
    let viewport_height = match app.app_mode() {
        AppMode::View { stack } => stack.last().map_or(viewport_height, |state| {
...
fn apply_mouse_capture(app: &App, terminal: &mut Terminal<CrosstermBackend<Stderr>>) {
    ...
    loop {
        loop {
            match rx.try_recv() {
                ...
                Ok(LoaderMessage::SearchBatch(pairs)) => {
                    app.apply_search_data(pairs);
                }
                Ok(LoaderMessage::SearchDone) => {
        ...
        let event_result = handle_events(
            ...
            |app, action| match action {
                ...
                Action::ToggleMouse => {
                    apply_mouse_capture(app, &mut guard.terminal);
                    EventLoopResult::Continue
                }
