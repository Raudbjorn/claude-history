...
pub enum LoaderMessage {
    ...
    /// Loading completed — list is usable, search data loading starts now
    Done,
    /// Full text for a batch of conversations (path, full_text), sent after Done
    SearchBatch(Vec<(PathBuf, String)>),
    /// All search data has been loaded — search index is now complete
    SearchDone,
}
