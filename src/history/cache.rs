...
//!
//! Stores display metadata in bincode, keyed by session filename and validated
//! by mtime + file size. Full text lives in a separate search cache and is
//! hydrated only by callers that need it.
...
const SCHEMA_VERSION: u32 = 10; // v10: cache invalidation for search split and unknown entries

const SEARCH_CACHE_MAGIC: [u8; 8] = *b"CLSRCH01";
...
/// Cached conversation data — display metadata only.
/// full_text and search_text_lower are stored separately in the search cache.
...
/// Cached full text for a session — stored separately to allow lazy loading.
#[derive(Serialize, Deserialize, Clone)]
...
// ── Search cache (full_text only, loaded lazily after display data) ──────────

#[derive(Serialize, Deserialize)]
struct SearchProjectCache {
    magic: [u8; 8],
    schema_version: u32,
    entries: HashMap<String, SearchCacheEntry>,
}

fn search_cache_path_for_project(project_dir_name: &str) -> Option<PathBuf> {
    cache_dir().map(|d| d.join(format!("{}.search.bin", project_dir_name)))
}

pub fn read_project_search_cache(
    project_dir_name: &str,
) -> Option<HashMap<String, SearchCacheEntry>> {
    let path = search_cache_path_for_project(project_dir_name)?;
    let data = std::fs::read(&path).ok()?;
    if data.len() < 12 || data[..8] != SEARCH_CACHE_MAGIC {
        return None;
    }
    let cache: SearchProjectCache = bincode::deserialize(&data).ok()?;
    if cache.schema_version != SEARCH_SCHEMA_VERSION {
        return None;
    }
    Some(cache.entries)
}

pub fn write_project_search_cache(
    project_dir_name: &str,
    entries: HashMap<String, SearchCacheEntry>,
) {
    let Some(path) = search_cache_path_for_project(project_dir_name) else {
        return;
    };
    let Some(parent) = path.parent() else {
        return;
    ...
    let _ = std::fs::create_dir_all(parent);
    let cache = SearchProjectCache {
        magic: SEARCH_CACHE_MAGIC,
...
#[cfg(test)]
pub(crate) fn remove_project_caches(project_dir_name: &str) {
    for path in [
        cache_path_for_project(project_dir_name),
        search_cache_path_for_project(project_dir_name),
    ]
    .into_iter()
    .flatten()
    {
...
/// Build a SearchCacheEntry from a full_text string and file metadata.
pub fn search_entry_from_full_text(
    full_text: &str,
    file_size: u64,
    mtime: SystemTime,
) -> SearchCacheEntry {
    ...
    SearchCacheEntry {
        ...
        mtime_nsecs: dur.subsec_nanos(),
        full_text: full_text.to_owned(),
    }
}

/// Check if a SearchCacheEntry matches the given file metadata.
pub fn search_entry_matches(entry: &SearchCacheEntry, file_size: u64, mtime: SystemTime) -> bool {
    let dur = mtime.duration_since(UNIX_EPOCH).unwrap_or_default();
    entry.file_size == file_size
        && entry.mtime_secs == dur.as_secs()
        && entry.mtime_nsecs == dur.subsec_nanos()
}
...
/// Create a CacheEntry from a parsed Conversation (display metadata only)
...
/// full_text and search_text_lower are left empty — filled in later by search loading.
...
mod tests {
    ...
    fn roundtrip_entry_preserves_data() {
        ...
        // full_text and search_text_lower are not stored in display cache
        ...
        assert_eq!(restored.search_text_lower, "");
    ...
    #[test]
    fn search_cache_roundtrip() {
        let project_name = format!("test-search-roundtrip-{}", std::process::id());
        let mtime = UNIX_EPOCH + Duration::from_secs(1700000000);
        let full_text = "Hello world this is a test conversation".to_string();
        let entry = search_entry_from_full_text(&full_text, 42000, mtime);

        assert!(search_entry_matches(&entry, 42000, mtime));
        assert!(!search_entry_matches(&entry, 42001, mtime));

        let mut entries = HashMap::new();
        entries.insert("conv1.jsonl".to_string(), entry);
        write_project_search_cache(&project_name, entries);

        let loaded = read_project_search_cache(&project_name).unwrap();
        assert_eq!(loaded.len(), 1);
        assert_eq!(loaded["conv1.jsonl"].full_text, full_text);

        if let Some(path) = search_cache_path_for_project(&project_name) {
            let _ = std::fs::remove_file(path);
        }
    }
    ...
    fn cache_file_roundtrip() {
        ...
        // Search text is stored separately; agent search data remains in display cache.
